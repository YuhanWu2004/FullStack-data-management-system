...
export default {
    ...
    async fetchAllPrograms({ commit }) {
        ...
        try {
            const response = await apiFetch(`${API_URL}?page=0&size=10000`)  // large size
            const data = await response.json()

            commit('SET_ALL_PROGRAM', data.programs)
        } catch (error) {
            commit('SET_ERROR', error.message || 'Failed to load all programs')
        }
    },
    async fetchPrograms({ commit, state }, { page, size, searchName, searchId } = {}) {
        ...
        try {
            ...
            commit('SET_ERROR', error.message || 'Failed to load programs')
        } finally {
    ...
    async createProgram({ commit }, programData) {
        ...
        try {
            ...
            commit('SET_ERROR', error.message || 'Failed to create program')
        } finally {
            commit('SET_LOADING', false)
        }
    },
    ...
    async updateProgram({ commit }, programData) {
        ...
        try {
            ...
            commit('SET_ERROR', error.message || 'Failed to update program')
        } finally {
            commit('SET_LOADING', false)
        }
    },
    ...
    async deleteProgram({ commit }, id) {
        ...
        try {
            ...
            commit('SET_ERROR', error.message || 'Failed to delete program')
        } finally {
            commit('SET_LOADING', false)
        }
    }
