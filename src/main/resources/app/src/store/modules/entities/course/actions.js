...
export default {
    ...
    async fetchAllCourses({commit}) {
        ...
        try {
            ...
            commit('SET_ERROR', error.message || 'Failed to load all courses')
        }
    },
    ...
    async fetchCourses({ commit, state }, { page, size, searchName, searchId } = {}) {
        ...
        try {
            ...
            commit('SET_ERROR', error.message || 'Failed to load courses')
        } finally {
    ...
    async deleteCourse({ commit }, courseId) {
        ...
        commit('SET_ERROR', null)
        try{
            await apiFetch(`${API_URL}/${courseId}`, {
                method: 'DELETE'
            })
            commit('DELETE_COURSE', courseId)
        } catch (error) {
            commit('SET_ERROR', error.message || 'Failed to delete course')
        } finally {
            commit('SET_LOADING', false)
        }
    }
